...
class GUISchemaBuilder:
    ...
    def _get_class_properties_with_metadata(self, class_uri: str) -> List[PropertyMetadata]:
        ...
        for result in results:
            ...
            prop_metadata = PropertyMetadata(
                uri=result['property'],
                name=result.get('propertyName', ''),
                display_name=result.get('displayName', result.get('shapeDisplayName', '')),
                form_group=result.get('formGroup', 'General'),
                display_order=int(result.get('displayOrder', 999)),
                data_type=self._determine_detailed_data_type(result),
                is_functional=bool(result.get('isFunctional', False)),
                is_required=bool(result.get('isRequired', False)),
                valid_values=[],  # Will be populated separately if needed
            ...
            # Add QUDT unit information if this is a measurement property
            if prop_metadata.data_type in ['double', 'float', 'integer'] and prop_metadata.default_unit:
    ...
    def _determine_detailed_data_type(self, result: Dict[str, Any]) -> str:
        """Determine detailed data type from query results."""
        if result.get('dataType') == 'object':
            return 'object'
        elif result.get('shapeDataType'):
            return self.sparql.normalize_data_type(result['shapeDataType'])
        else:
