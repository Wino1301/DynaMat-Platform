...
class CalculationEngine:
    ...
    def _calc_circular_area_from_diameter(self, diameter: float, **kwargs) -> float:
    ...
    def _calc_rectangular_area(self, length: float, width: float, **kwargs) -> float:
        """Calculate rectangular area: A = length × width"""
        if length <= 0 or width <= 0:
            raise ValueError("Length and width must be positive")
        return length * width
    
    def _calc_square_area(self, side: float, **kwargs) -> float:
